...
def add_track(track_data: Dict[str, Any]) -> None:
...
def get_all_tracks() -> List[Dict[str, Any]]:
    ...
    with create_connection() as conn:
        ...
        try:
            conn.row_factory = (
                sqlite3.Row
            )  # Devuelve filas que se pueden acceder por nombre de columna
            cursor = conn.cursor()
...
def update_track_field(file_path: str, field: str, value: Any) -> None:
    ...
    with create_connection() as conn:
        ...
        try:
            ...
            conn.commit()
            print(
                f"Base de datos actualizada para {os.path.basename(file_path)}: {field} = {value}"
            )
        except sqlite3.Error as e:
